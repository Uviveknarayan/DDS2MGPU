...
class VS2M(object):
    ...
    def _init_inputs(self):
        ...
        original_noise = torch_to_np(get_noise1(1, 'noise', (32, self.image_clean.shape[0], self.image_clean.shape[1]), noise_type='u',
                                                                     var=10/10.).type(torch.cuda.FloatTensor).detach())
        self.ESSA_net_inputs = np_to_torch(original_noise).type(torch.cuda.FloatTensor).detach()[0, :, :, :]
    ...
    def _optimization_closure(self, at, j):
        ...
        self.ESSA_out = out[0, :, :, :].squeeze().reshape((-1, 1)).reshape(65536,32)
        self.ESSA_out_np=torch_to_np(self.ESSA_out)
        self.mask_out_np = torch_to_np(self.mask_out)
        self.image_com = self.image_out.mm(self.mask_out)
        self.image_com=self.image_com+self.ESSA_out
        self.image_com_np = np.matmul(self.image_out_np, self.mask_out_np)+self.ESSA_out_np
